Then('the welcome message should be visible before logout') do
  ...
  expect(page).to have_selector(:css, '#nameofuser', wait: 10)

  actual_message = find(:css, '#nameofuser', wait: 10).text
...
When('I click on Log out for logout validation') do
  expect(page).to have_selector(:css, '#logout2', wait: 10)

  find(:css, '#logout2', wait: 10).click
